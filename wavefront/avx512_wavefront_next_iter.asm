...
avx512_wavefront_next_iter1:
    ...
    mov eax, 1
    ;MAX() + 1
    vmovdqa32 zmm0, [rdi]; dest, src [int32_t]
    vmovdqa32 zmm1, [rsi]
    ...
    vmovdqa32 [rdx], zmm3 ;assignemnt to out_i1

    ;MAX() + 0
    vmovdqa32 zmm0, [rcx] ;destructive
    vmovdqa32 zmm1, [r8]
    vpmaxsd zmm4, zmm1, zmm0 
    
    vmovdqa32 [r9], zmm4 ;assignment to out_d1
    ret
...
;final MAX(del1, MAX(misms, ins1))
    ...
    vmovdqa32 [rcx], zmm4 ;return to max
...
avx512_wavefront_next_iter3:
    ...
    ;use vmovdqa32 with mask
    mov eax, 0xC0000000; nullvalue entry
    vpbroadcastd zmm5, eax ;broadcast null value
    vmovdqa32 zmm0 {k1}, zmm5
    ;vmovdqa32 zmm0 {k2}, zmm5

    vmovdqa32 [rsi], zmm0 ;assign output to out_m
